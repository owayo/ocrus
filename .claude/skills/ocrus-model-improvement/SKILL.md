...
| 指標 | 値 |
| --- | --- |
| ひらがな / カタカナ（845 枚） | 92.8% / 83.5%（合計 **88.0%**） |
| 常用漢字（2,136 枚・1 フォント） | **97.5%** |
| 半角英数 / 全角英数 | 77.1% / 79.4% |
| 半角記号 / 全角記号 | 50.0% / 52.7% |
...
- **孤立文字なら、まず切り抜き方を疑う** — 2026-09-03 の改善はすべてそこだった。
  画線が離れた字を別々の行として切る、小書きかなを大きさで判断する、極小の記号を
  拡大する。いずれも「インクに密着させて切る」既定が情報を捨てていた（`todo.md`）。
