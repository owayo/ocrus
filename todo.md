...
- **原理**: `,` `.` `'` `_` `:` は数ピクセルのインクしかない。それを切り抜いて
  モデルの固定高さまで拡大すると別物の塊になり、`.` は `a`、`,` は `l` と読まれる。
  インクが枠の 20% 以下なら枠のまま渡し、「小さくて下にある」という情報を残す
- **コード**: `keep_frame_for_tiny_mark()`
- **しきい値の選び方**: 記号だけ見れば 0.35 が最良（63.2%）だが、かなが 83.6% まで落ちる。
  件数で重み付けした合計が最大になる **0.20** を採った（記号 58.9% / かな 88.0%、1 フォント）

### 全カテゴリの最終数値（2026-09-03、本番パイプライン、5 フォント）

| カテゴリ | いま | 2026-03 の記録（評価経路） |
|---|---|---|
| ひらがな | **92.8%** | 76.9% |
| カタカナ | **83.5%** | 68.6% |
| 常用漢字（1 フォント） | **97.5%** | 未測定 |
| 半角英数 | **77.1%** | 76.8% |
| 全角英数 | **79.4%** | 77.4% |
| 半角記号 | **50.0%** | 41.2% |
| 全角記号 | **52.7%** | 39.0% |

`--mode accurate` でさらに +0.5pt、`--charset jis` 併用で +0.8pt（かなで測定）。

### ❌ 全角/半角を枠の縦横比で判定（ベンチマークへの過学習なので採らない）

記号の失敗 229 件は、`＿→—`、`〜→~`、`［→「`、`+→十` のような
**全角/半角・同形ペア**が大半を占める。分離できる信号があるか測った。

| 指標 | 全角 | 半角 |
|---|---|---|
| インク幅 / 枠高 | 中央 0.19〜0.30 | 中央 0.18〜0.23（**重なっていて使えない**） |
| 枠の縦横比 | 1.01 | 0.74（**分離できる**） |

枠の縦横比なら分離できるが、これは**テスト画像が 1 文字ずつ em ボックスに描かれている
という描画規約**に依存している。実際の切り出し画像がその規約に従う保証はないので、
ベンチマークの点数は上がっても実用では当てにならない。**採用しない。**

### オプションの効き方（一律ではない）

| 条件 | かな 845 枚 | 記号 475 枚 |
|---|---|---|
| 既定 | 88.0% | 51.8% |
| `--charset jis` | 88.3% 相当 | **52.8%** |
| `--mode accurate` | **88.5%** | 50.7%（**悪化**） |

`--mode accurate` の前処理バリアントは、細らせる変換で細い記号が消えるため記号に不利。
かなには効く。**一律に良いオプションではない**ので、既定を変えるときは両方測ること。

### 今回分かった一般則

**孤立文字では「切り抜き方」が精度を決める。** 3 つの修正はすべて同じ話だった。

1. 画線が離れた字を別々の行として切ってはいけない（う、こ、き）
2. 小書きかなは大きさではなく**枠の中の上下位置**で見分ける
3. 極小の記号は切り抜かず**枠のまま**渡す

いずれも「インクに密着させて切る」という既定が、孤立文字では情報を捨てていた。
